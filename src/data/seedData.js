...
export const TRAINING = [
  { title: '房間清潔流程', intro: '入房到報房的標準次序：', steps: ['敲門報名 → 進房開窗通風', '收布草、倒垃圾', '鋪床（四角包實）', '由高到低、由裡到外抹塵', '浴室清潔（最後檢查排水口）', '補品對清單', '吸塵由裡向門口退出', '最後360°目視檢查', '報房更新房態'] },
  { title: '衛生標準', intro: '衛生是房務的底線：', steps: ['布草逐件檢查，污漬破損即換', '杯具統一回工作間消毒', '抹布分色使用不交叉', '化學品按標籤用途及比例'] },
  { title: 'DND 處理', intro: '掛牌房的處理原則：', steps: ['開工先核對DND名單', 'DND房不敲門不打擾', '下午再確認一次', '過夜DND上報主管跟進'] },
  { title: 'Stayover 程序', intro: '住客房清潔的分寸：', steps: ['客人物品原位不動', '私人文件、財物不觸碰', '床鋪整理、毛巾按使用情況更換', '發現異常（損壞/違禁品）即報主管'] },
  { title: 'Checkout 程序', intro: 'C/O房是深度檢查房：', steps: ['電筒照床底、梳化底、抽屜', '夾萬確認已開已空', '迷你吧點算入賬', '遺留物即報即登記', '全面更換布草及消耗品'] },
  { title: 'Lost & Found', intro: '遺留物處理五步：', steps: ['發現 → 即通知主管', '即日登記（日期/房號/物品/發現人）', '拍照存檔', '按類別上架保存', '貴重物品雙人處理、密封簽名'] },
  { title: 'VIP 房要求', intro: 'VIP房的加碼標準：', steps: ['按VIP等級對照擺設清單', '歡迎品、鮮花、卡片檢查', '主管必須複查後才報房', '留意客史偏好（枕頭/樓層/忌諱）'] },
  { title: '客人服務禮儀', intro: '每一次接觸都是服務：', steps: ['主動微笑、稱呼客人', '走廊靠邊讓路、電梯讓客人先', '聽不懂的要求找主管，不亂應承', '不議論客人、不透露房號住客資料'] },
  { title: '安全事項', intro: '保護自己也保護客人：', steps: ['化學品不混用、不徒手接觸', '彎腰搬重物用腳力不用腰力', '發現可疑人物/物品即報', '熟悉走火通道及滅火器位置'] },
  { title: '工作車整理', intro: '車的標準：', steps: ['上層：清潔劑及工具', '中層：備品消耗品', '下層：乾淨布草', '側掛：布草袋、垃圾袋（不過2/3滿）', '車靠牆停、不擋通道'] },
  { title: '工作間整理', intro: '後場標準：', steps: ['乾淨/污穢布草嚴格分區', '化學品上鎖存放', '消耗品每日點數', '收工前清潔地面'] },
  { title: '常見問題', intro: '新人最常中招位：', steps: ['趕房漏最後檢查 → 寧慢一分鐘', '憑記憶補品 → 必須對清單', '見DND照敲 → 先看名單', '遺留物袋住先 → 即報即登記', '杯具房內求其抹 → 統一消毒'] },
]
